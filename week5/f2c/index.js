...
app.get("/", function (req, res) {
  res.sendFile(__dirname + "/f2cCalc.html"); //changed to correct file name
});

// this gets the response from the values in the web page
app.post("/", function (req, res) {
  var fTemp = Number(req.temp.n1);

  // does the computation of the input variables, as numbers
  var result = fTemp;

  // sends the results back to the web page as string
  res.send("The result of the calculation is " + result + " C");
});
//this gets the response from the web page to this placeholder

// this code is only invoked on the path /f2cCalculator
app.get("/f2cCalculator", function (req, res) {
...
app.post("/f2cCalculator", function (req, res) {
  ...
  var fTemp = parseFloat(req.temp.n1);
  

  // does the computation of the input variables, as numbers
  var f2c = (fTemp) - 32) x 5/9;

  //display the result in 2 decimal places
  res.send(fTemp + "Fahrenheit converted to Centigrade is " + f2c.toFixed(2));
});
